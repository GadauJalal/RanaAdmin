...
- This is a front-end prototype with no production backend, identity provider, notification service, gateway service, or database.
- Seed data is in `data.js`.
- Routing, rendering, forms, validation, state transitions, search, and exports are in `app.js`.
- Visual rules are in `styles.css`.
- Official assets and fonts are in `assets/` and `fonts/`.
- Changes persist in browser `localStorage` for the current browser only.
- CSV files are generated in the browser.
- Hash routes support direct navigation.

## Files

- `index.html`: application entry point
- `styles.css`: responsive design system and layout
- `data.js`: prototype network data
- `app.js`: routing, views, forms, validation, state transitions, and audit behavior
- `assets/rana54-mark.png`: official mark used in the interface
